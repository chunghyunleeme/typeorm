...
TypeORM에서는 양방향으로 만들어야 하고 orphanRemoval이 없다.
...
```typescript
// TypeORM — 직접 삭제해야 함
const removedImage = article.images.splice(0, 1)[0]
await manager.remove(removedImage) // 명시적 삭제 필요
await manager.save(article)
```

TypeORM의 `cascade: true`는 부모 저장 시 자식을 INSERT/UPDATE하지만,
컬렉션에서 빠진 자식을 자동으로 DELETE하지는 않는다.
`orphanRemoval`은 영속성 컨텍스트가 컬렉션 상태를 추적해야 가능한데,
TypeORM에는 영속성 컨텍스트가 없으므로 구현할 수 없다.
...
| **orphanRemoval**        | `orphanRemoval = true`                  | 없음 — 직접 `remove()` 호출                                 |
